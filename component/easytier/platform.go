...
func (s SocketFactory) ListenTCP(ctx context.Context, options platform.TCPListenOptions) (net.Listener, error) {
	...
	network := tcpNetwork(options.Bind)
	// TCP reservations bind locally; the current mihomo dialer API exposes packet listeners only.
	if _, ok := s.Dialer.(dialer.Dialer); !ok && externalTCPListen(options.Purpose) {
